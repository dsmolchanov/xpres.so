...
function App() {
  ...
  const navigateToFrame = useCallback((frameIndex: number, skipAnimation: boolean = false) => {
    ...
    if (isPresenting && !skipAnimation) {
      ...
      const animationInterval = setInterval(() => {
        ...
        const currentScrollX = startScrollX + (targetScrollX - startScrollX) * easeProgress;
        const currentScrollY = startScrollY + (targetScrollY - startScrollY) * easeProgress;
  ...
  }, [isPresenting, nextFrame, previousFrame, goToFirstFrame, goToLastFrame, togglePresentation]);

  // Handle PDF export
  const handleExportPDF = useCallback(async () => {
    if (frames.length === 0) {
      alert('No frames to export. Please add frames using the Frame tool (F key).');
      return;
    }
